"""
benchmark_35model_moe.py — 全35モデル投入MoEベンチマーク
================================================================
全ステージ（model, guitarset_ft, multitask, multitask_3ds, multitask_3ds_ga）
× 7ドメインの35モデルを合議させ、vote_threshold 10-25をスイープする。

Usage:
    python benchmark_35model_moe.py
"""
...
def main():
    ...
    results = []
    for vote_threshold in range(10, 26):
        total_tp_pitch = 0
        total_tp_sf = 0
        total_fp = 0
        total_fn = 0
    ...
    print(f"  比較: 7-model MoE F1=0.8877 (Synth V2) / 0.8839 (Step 6)")
    print(f"{'='*80}")

    # 結果保存
    out_path = os.path.join(r'D:\Music\nextchord-solotab\backend\benchmark', '35model_moe_sweep_results.json')
