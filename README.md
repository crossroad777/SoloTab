...
## SoloTab v1.0 — 安定版リリースノート

### コアパイプライン（F1=0.839）
- 弦/フレット割り当て（360曲ベンチマーク、Regressionゼロ）
- 運指割り当て（クリーンデータ 96.88%）
- リズム量子化、タイ分割、サステイン保護
- 文脈依存の開放弦ボーナス減衰（DADGADドローン保護含む）

### Human-in-the-Loop
- 1点アンカー制約付き Viterbi 再計算（ドミノ倒しUX）
- テクニック記号の手動指定UI

### テクニック記号
- 自動検出: slide, bend, vibrato, harmonic（IDMT検証 Precision 1.00）
- 手動指定: 上記4種 + H/P（ユーザーによる指定のみ）

### 既知の限界（ADR参照）
- H/P の自動検出は記号データの構造的限界により除外（ADR-003）
