"""
analyze_gp_treasures_v3.py
==========================
堅牢なGuitarProパーサーによる完全統計・宝判定。
"""
...
for p in sample:
    ...
    try:
        ...
        if song.tracks:
            t1 = song.tracks[0]
            t_vals = [s.value for s in t1.strings] if getattr(t1, "strings", None) else []
            if len(t_vals) == 6:
                ...
                tuning_str = f"{len(t_vals)}-String" if t_vals else "Unknown"
            ...
            for m in track.measures:
                ...
                for voice in m.voices:
                    for beat in voice.beats:
                        ...
                        if dur and getattr(dur, "tuplet", None):
                            tup = dur.tuplet
                        ...
                        for note in getattr(beat, "notes", []):
                            ...
                            if eff:
                                ...
                                if (lh and lh != guitarpro.Fingering.none) or (rh and rh != guitarpro.Fingering.none):
                                    file_fingered += 1
...
print("\n" + "="*60)
print(f"総ファイル数: {total_files:,} 件 / 解析成功率: {success/len(sample)*100:.1f}%")
