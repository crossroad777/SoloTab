"""
analyze_gp_treasures_v2.py
==========================
26,092件のGuitar Proコレクションから
正確なメタデータ・運指率・ポリフォニー率・奏法記号・3連符を抽出する。
"""
...
# ──────────────────────────────────────────────────────────
# Step 2 & 3: 200ファイルサンプリング解析
# ──────────────────────────────────────────────────────────
print("\n=== Step 2 & 3: 200曲サンプリング解析（宝判定） ===")
...
print(f"サンプリング対象: {len(sample)} 件 (クラシック: {len(classical_files[:60])}, フィンガースタイル: {len(fingerstyle_files[:40])}, ロック/ポップ/その他: {len(sample)-len(classical_files[:60])-len(fingerstyle_files[:40])})")
    ...
    try:
        ...
        for track in song.tracks:
            for m in track.measures:
                if len(m.voices) >= 2:
                    v0 = any(len(b.notes) > 0 for b in m.voices[0].beats if getattr(b, "notes", None))
                    v1 = any(len(b.notes) > 0 for b in m.voices[1].beats if getattr(b, "notes", None))
                    if v0 and v1:
                        has_poly = True

                for voice in m.voices:
                    for beat in voice.beats:
                        if hasattr(beat, "duration") and hasattr(beat.duration, "tuplet"):
                            tup = beat.duration.tuplet
                            if tup and (tup.enters != 1 or tup.times != 1):
                                has_tup = True
                        ...
                        for note in beat.notes:
                            ...
                            eff = note.effect
                            if eff:
                                if eff.leftHandFinger != guitarpro.Fingering.none or eff.rightHandFinger != guitarpro.Fingering.none:
                                    file_fingered += 1
                                if eff.hammer: file_techs["hammer_pull"] += 1
                                if eff.slides: file_techs["slide"] += 1
                                if eff.bend: file_techs["bend"] += 1
                                if eff.harmonic: file_techs["harmonic"] += 1
                                if eff.vibrato: file_techs["vibrato"] += 1
                                if eff.ghostNote: file_techs["ghost_note"] += 1
...
print(f"\n[解析結果]")
print(f"成功: {success} / {len(sample)} ({success/len(sample)*100:.1f}%)")
print(f"失敗: {failed} (エラー: {dict(errors)})")
