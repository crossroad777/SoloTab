...
matching = mir_eval.transcription.match_notes(ref_int, ref_p, est_int, est_p, onset_tolerance=0.05, pitch_tolerance=50.0)
...
print("First few GT notes:")
for i in range(min(5, len(ref_p))):
    print(f"GT[{i}]: pitch={ref_p[i]}, onset={ref_int[i][0]:.4f}, offset={ref_int[i][1]:.4f}")

print("\nFirst few PRED notes:")
for i in range(min(5, len(est_p))):
    print(f"PR[{i}]: pitch={est_p[i]}, onset={est_int[i][0]:.4f}, offset={est_int[i][1]:.4f}")
